...
# ========================================
    ...
    if st.button("🚀 Train & Lưu mô hình luôn"):
        ...
        # 👉 LƯU NGAY, KHÔNG DÙNG session_state
            joblib.dump(best_model, 'models/best_model.pkl')
            joblib.dump(scaler, 'models/scaler.pkl')
            joblib.dump(label_encoders, 'models/label_encoders.pkl')
            joblib.dump(feature_names, 'models/feature_names.pkl')

            st.success(f"✅ Đã train & lưu model: {best_name}")
...
# ========================================
    ...
    try:
        ...
        st.markdown("---")
        st.subheader("📝 Nhập thông tin khảo sát của bạn")
        ...
        with st.form("prediction_form"):
            ...
            with col1:
                age = st.number_input("Độ tuổi", min_value=10, max_value=25, value=18)
                gender = st.selectbox("Giới tính", options=["Male", "Female"])
                gpa = st.number_input("Điểm trung bình (GPA)", min_value=0.0, max_value=4.0, value=3.2, step=0.1)
                sleep_hours = st.slider("Số giờ ngủ/ngày", 0.0, 14.0, 7.0, step=0.5)
                physical_activity = st.slider("Hoạt động thể chất (giờ/tuần)", 0.0, 20.0, 3.0, step=0.5)
                
            with col2:
                screen_time = st.slider("Thời gian dùng thiết bị (giờ/ngày)", 0.0, 24.0, 5.0, step=0.5)
                social_media = st.slider("Thời gian dùng MXH (giờ/ngày)", 0.0, 24.0, 3.0, step=0.5)
                stress_level = st.slider("Mức độ Stress (0-10)", 0, 10, 5)
                anxiety_level = st.slider("Mức độ Lo âu (0-10)", 0, 10, 5)
            ...
            with col3:
                platform = st.selectbox("Nền tảng sử dụng", ["Instagram", "TikTok", "Both"])
                interaction = st.selectbox("Mức độ tương tác xã hội", ["low", "medium", "high"])

            with col4:
                addiction = st.slider("Mức độ nghiện (0-10)", 0, 10, 5)
            submitted = st.form_submit_button("🤖 Dự đoán kết quả")

        if submitted:
            # Chuyển dữ liệu thành DataFrame
            input_data = pd.DataFrame({
                'age':[age], 'gender':[gender], 'academic_performance':[gpa],
                'sleep_hours':[sleep_hours], 'physical_activity':[physical_activity],
                'screen_time_before_sleep':[screen_time], 'daily_social_media_hours':[social_media],
                'stress_level':[stress_level], 'anxiety_level':[anxiety_level],
                'platform_usage':[platform],'social_interaction_level':[interaction],
                'addiction_level':[addiction]
            })

            # Xử lý Encoding & Scaling (Phải khớp với quá trình Train)
            ...
            for col in feature_names:
                if col not in input_data.columns:
                    input_data[col] = 0
            # ========================
            input_data = input_data[feature_names] # Đồng bộ cột
            ...
            st.markdown("---")
            if pred[0] == 1:
                st.error(f"⚠️ CẢNH BÁO: Có nguy cơ trầm cảm (Xác suất: {prob:.1%})")
                st.write("Lời khuyên: Bạn nên dành thời gian nghỉ ngơi, giảm bớt sử dụng thiết bị điện tử và tham khảo ý kiến chuyên gia.")
            else:
                st.success(f"✅ AN TOÀN: Trạng thái tâm lý ổn định (Xác suất rủi ro: {prob:.1%})")
                st.balloons()

    except:
        st.warning("⚠️ Bạn cần huấn luyện mô hình ở tab 'Xây dựng mô hình' trước khi dự đoán!")
